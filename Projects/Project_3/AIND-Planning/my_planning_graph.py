...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            if a_node.prenodes.issubset(self.s_levels[level]):
                for an, sn in zip(a_node.prenodes, self.s_levels[level]):
                    if an.symbol == sn.symbol:
                        sn.children.add(an)
                        an.parents.add(sn)
    ...
    def add_literal_level(self, level):
        ...
        for a_node in self.a_levels[level-1]:
             for e_node in a_node.effnodes:
                 s_node = PgNode_s(e_node.symbol, e_node.is_pos)
                 s_node.parents.add(a_node.children.add(s_node))
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        effects_a1_pos = set(node_a1.action.effect_add)
        effects_a1_neg = set(node_a1.action.effect_rem)
        effects_a2_pos = set(node_a2.action.effect_add)
        effects_a2_neg = set(node_a2.action.effect_rem)

        if effects_a1_pos.intersection(effects_a2_neg) \
        or effects_a2_pos.intersection(effects_a1_neg):
            return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        if effects_a1_pos.intersection(precond_a2_neg) \
        or effects_a1_neg.intersection(precond_a2_pos) \
        or effects_a2_pos.intersection(precond_a1_neg) \
        or effects_a2_neg.intersection(precond_a1_pos):
            return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for n1, n2 in zip(node_a1.parents, node_a2.parents):
            if n1.is_mutex(n2):
                return True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        all_mutex = True
        both_states = False
        for n1, n2 in zip(node_s1.parents, node_s2.parents):
            if n1 and n2:
                if not n1.is_mutex(n2):
                    all_mutex = False
                if node_s1.symbol in n1.action.effect_add and node_s2.symbol in n1.action.effect_add \
                or node_s1.symbol in n2.action.effect_add and node_s2.symbol in n2.action.effect_add \
                or node_s1.symbol in n2.action.effect_rem and node_s2.symbol in n2.action.effect_rem \
                or node_s1.symbol in n2.action.effect_rem and node_s2.symbol in n2.action.effect_rem:
                    both_states = True

        if all_mutex and not both_states:
            return True
        ...
        for goal in self.problem.goal:
            ...
            for level in range(len(self.s_levels)):
                for node in self.s_levels[level]:
                    if node.symbol == goal and node.is_pos and not goal_found:
                        level_sum += level
                        goal_found = True
                    if goal_found: pass
                if goal_found: pass
